...
include_once libfile('function/ftp');
...
if(!($operation)) {
	...
	if(!submitcheck('upgradesubmit', 1) && !submitcheck('deletesubmit')) {
		?>
		<script src="<?=$_G['setting']['upgradeurl']?>?release=<?=DISCUZ_RELEASE?>&charset=<?=CHARSET?>&time=<?=time()?>"></script>
		<?
		...
		while(($file = readdir($dh)) != false) {
			if($file != '.' && $file != '..' && preg_match('/^[0-9]+$/', $file) && is_dir($backuppath.'/'.$file)) {
				$backupinfo .= "<tr><td><input class=\"checkbox\" type=\"checkbox\" name=\"delete[]\" value=\"$file\"></td>\n".
					"<td>$file</a></td>\n".
					"<td>".dgmdate(filemtime($backuppath.'/'.$file))."</td>\n";
			}
		}
		closedir($dh);
		...
		<br />
		<form method="post" autocomplete="off" action="<?=ADMINSCRIPT?>?action=upgrade">
		...
		?>
		<script src="<?=$_G['setting']['upgradeurl']?>?release=<?=DISCUZ_RELEASE?>&time=<?=time()?>"></script>
		...
		<script type="text/JavaScript">
			...
			function upgrade() {
				...
				$('msg').innerHTML = '<?=$lang['upgrade_closesite']?>';
					x = new Ajax('HTML', '');
					x.get(ADMINSCRIPT.'?action=upgrade&operation=openbbs&bbc=1', function(s) {
			...
			function upgradefile() {
				...
				x.get(ADMINSCRIPT.'?action=upgrade&operation=down&release='+upgradelist[i].release+'&id='+j+run, function(s) {
					...
					if( j < upgradepackage[i].update.length) {
						...
						if( i < upgradepackage.length) {
							...
							x = new Ajax('HTML', '');
							x.get(ADMINSCRIPT.'?action=upgrade&operation=openbbs&bbc=0', function(s) {
	...
	if(!($ftpid = dftp_connect($ftphost, $ftpuser, $ftppass, $ftppath, $ftpport, $ftpssl))) {
		...
		if(!$type) {
			...
			if(!($fp = fopen($_G['setting']['upgradeurl'].$release.'/package/'.$script.'.txt', 'rb'))) {
				exit(false);
			}
