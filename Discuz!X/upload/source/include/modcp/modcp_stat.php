...
for($i = 0; $i <= $_G['setting']['maxmodworksmonths']; $i++) {
	...
	if($i != $before) {
		$monthlinks[$i] = "<a href=\"forum.php?mod=modcp&action=stat&before=$i&uid=$uid\" hidefocus=\"true\" class=\"brw\">$month</a>";
	} else {
		$thismonth = $month;
		$starttime = $month.'-01';
		$endtime = date("Y-m-01", mktime(0, 0, 0, $now['month'] - $i + 1 , 1, $now['year']));
		$daysofmonth = date("t", mktime(0, 0, 0, $now['month'] - $i , 1, $now['year']));
		$monthlinks[$i] = "<a href=\"forum.php?mod=modcp&action=stat&before=$i&uid=$uid\" hidefocus=\"true\" class=\"brw otp\">$month</a>";
	}
}
