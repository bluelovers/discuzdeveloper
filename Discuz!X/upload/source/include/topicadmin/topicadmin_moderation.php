...
if($tids = dimplode($_G['gp_moderate'])) {
	foreach($threadtableids as $tableid) {
		...
		$query = DB::query("SELECT * FROM ".DB::table($threadtable)." WHERE tid IN ($tids) AND fid='$_G[fid]' AND displayorder>='0' AND digest>='0' LIMIT $_G[tpp]");
	...
	if(empty($operations)) {
		...
		foreach($operations as $operation) {
			...
			if($operation == 'stick') {
				...
				$forumstickthreads = DB::result_first("SELECT svalue AS value FROM ".DB::table('common_setting')." WHERE skey='forumstickthreads'");
				...
				if(DB::fetch_first("SELECT * FROM ".DB::table('forum_forumrecommend')." WHERE tid IN ($moderatetids)")) {
