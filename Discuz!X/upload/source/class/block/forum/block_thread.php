...
class block_thread {
		...
		while($data = DB::fetch($query)) {
			...
			$list[] = array(
				...
				'fields' => array(
					...
					'avatar' => avatar(($data['author'] ? $data['authorid'] : 0), 'small', true, false, $_G['setting']['avatarmethod'], $_G['setting']['ucenterurl']),
					'avatar_middle' => avatar(($data['author'] ? $data['authorid'] : 0), 'middle', true, false, $_G['setting']['avatarmethod'], $_G['setting']['ucenterurl']),
					'avatar_big' => avatar(($data['author'] ? $data['authorid'] : 0), 'big', true, false, $_G['setting']['avatarmethod'], $_G['setting']['ucenterurl']),
					'posts' => $data['posts'],
	...
	function getpic($tid) {
		...
		$pic = DB::fetch_first("SELECT attachment, remote FROM ".DB::table('forum_attachment')." WHERE tid='$tid' AND isimage IN (1, -1) ORDER BY dateline DESC LIMIT 0,1");
		return $pic;
	}
}
