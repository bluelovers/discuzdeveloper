...
if($_GET['step'] == 'start') {
	...
	if(empty($_GET['op']) || $_GET['op'] == 'realname') {// 实名功能
		...
		$nextop = 'updatethreadtype';// 下一个处理的功能
	...
	} elseif($_GET['op'] == 'updatethreadtype') { //升级分类信息
		$nextop = 'updatecron';
		$selectoption = array();
	...
	} elseif($_GET['op'] == 'updatemagic') { //升级道具
	...
	} elseif($_GET['op'] == 'portalcategory_permission') { //处理模块权限表
	...
	} elseif($_GET['op'] == 'portal_comment') { //处理文章评论
	...
	} elseif($_GET['op'] == 'portal_article_cover_img') { //处理文章评论
		$nextop = 'block_style';
	...
	} elseif($_GET['op'] == 'forumattach') {	//论坛附件表升级
		$nextop = 'moderate';
		$limit = 10000;
	...
	} elseif($_GET['op'] == 'threadimage') {	//主题图片表处理
		$nextop = 'forumattach';
		$defaultmonth = 10;
		$limit = 1000;
		$start = !empty($_GET['start']) ? $_GET['start'] : 0;
...
/*
 * 论坛/群组附件模块的样式转换为论坛/群组帖子模块的样式
 * @param type $style
 * @param type $blockclass
 * @return type $block
 */
