...
function updateswfattach() {
	global $db, $tablepre, $attachsave, $attachdir, $discuz_uid, $postattachcredits, $tid, $pid, $attachextensions, $swfattachnew, $swfattachdel, $allowsetattachperm, $maxprice, $updateswfattach, $watermarkstatus;
	...
	if($attachnum && $updateswfattach) {
		if($attachnum -= count($swfattachdel)) {
		...
		while($swfattach = $db->fetch_array($query)) {
			...
			if($swfattach['isimage'] && $watermarkstatus) {
				...
				if($image->imagecreatefromfunc && $image->imagefunc) {
					...
					$swfattach['filesize'] = $swfattach['size'];
				}
			}
			...
			$attachnew['filesize'] = $swfattach['filesize'];
			...
			$db->query("UPDATE {$tablepre}attachments SET tid='$tid', pid='$pid', attachment='$attachnew[attachment]', description='$attachnew[description]', readperm='$attachnew[readperm]', price='$attachnew[price]', remote='$attachnew[remote]', filesize='$attachnew[filesize]' WHERE aid='$swfattach[aid]'");
			$swfattachcount++;
		}
		if($delaids) {
