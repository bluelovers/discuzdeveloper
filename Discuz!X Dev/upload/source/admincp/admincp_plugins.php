...
if(!$operation) {
	...
	if(!submitcheck('submit')) {
		...
		showsubmenu('nav_plugins', array(
			array(array('menu' => 'plugins_list', 'submenu' => array(
				array('plugins_list_standard', 'plugins', empty($_G['gp_view'])),
				array('plugins_list_system', 'plugins&view=system', !empty($_G['gp_view'])),
			)), 1),
			array('plugins_install', 'plugins&operation=import', 0),
			$isplugindeveloper ? array('plugins_add', 'plugins&operation=add', 0) : array(),
		));
		showformheader('plugins'.(!empty($_G['gp_view']) ? '&view=system' : ''));
		showtableheader();
		showsubtitle(array('plugins_name', 'version', 'copyright', 'plugins_directory', 'display_order', '&nbsp;<a href="'.ADMINSCRIPT.'?action=plugins&operation=validator" style="font-weight:normal">'.$lang['plugins_validator'].'</a>'));
		...
		while($plugin = DB::fetch($query)) {
			...
			if(empty($_G['gp_view']) && $plugin['modules']['system'] == 2 || !empty($_G['gp_view']) && $plugin['modules']['system'] != 2) {
				continue;
			}
			...
			showtablerow('style="height:20px"', array(($plugin['available'] ? 'class="bold"' : ''), 'class="td23"', '', '', 'class="td28 td23"'), array(
				$plugin['modules']['system'] != 2 ? "<a href=\"".ADMINSCRIPT."?action=plugins&operation=config&do=$plugin[pluginid]\">".dhtmlspecialchars($plugin['name'])."</a>" : dhtmlspecialchars($plugin['name']),
				...
				($plugin['modules']['system'] != 2 ? (!$plugin['available'] ? "<a href=\"".ADMINSCRIPT."?action=plugins&operation=enable&pluginid=$plugin[pluginid]\" class=\"act\">$lang[enable]</a>&nbsp;" : "<a href=\"".ADMINSCRIPT."?action=plugins&operation=disable&pluginid=$plugin[pluginid]\" class=\"act\">$lang[closed]</a>&nbsp;") : '').
				(!$plugin['modules']['system'] ? "<a href=\"".ADMINSCRIPT."?action=plugins&operation=delete&pluginid=$plugin[pluginid]\" class=\"act\">$lang[plugins_config_uninstall]</a>&nbsp;" : "<a href=\"".ADMINSCRIPT."?action=plugins&operation=delete&pluginid=$plugin[pluginid]\" class=\"act\">$lang[plugins_config_upgrade]</a>&nbsp;").
		...
		cpmsg('plugins_edit_succeed', 'action=plugins'.(!empty($_G['gp_view']) ? '&view=system' : ''), 'succeed');
	...
	$conflictplugins = '';
	if($operation == 'enable' && empty($_G['gp_confirm'])) {
			...
			if(!in_array($entry, array('.', '..')) && is_dir($plugindir.'/'.$entry) && !in_array($entry.'/', $installsdir)) {
				...
				while($f = $d->read()) {
					if(preg_match('/^discuz\_plugin\_'.$entry.'(\_\w+)?\.xml$/', $f)) {
						...
						$file = $entrydir.'/'.$f;
						echo '<tr><td>'.$entrytitle.($filemtime > TIMESTAMP - 86400 ? ' <font color="red">New!</font>' : '').'</td><td>'.$entryversion.'</td><td>'.$entrycopyright.'</td><td>'.$entry.'/</td>'.
							'<td><a href="'.ADMINSCRIPT.'?action=plugins&operation=import&dir='.$entry.'&validator=yes" class="act">'.$lang['plugins_config_install'].'</a></td></tr>';
						break;
					}
				}
			}
	...
	if(!$_G['gp_confirmed']) {
		...
		echo '<div class="infobox">'.$addonstr.($upgradestr ? '<h4 class="infotitle2">'.$lang['plugins_config_upgrade'].'</h4>'.$upgradestr.'<br /><br />' : '').
			($deletestr ? '<h4 class="infotitle2">'.$lang['plugins_config_delete'].'</h4>'.$deletestr.'<br /><br />' : '<h4 class="infotitle2">'.$lang['plugins_config_upgrade_missed'].'</h4>').
	...
	$query = DB::query("SELECT identifier, name, version FROM ".DB::table('common_plugin')." WHERE $where");
	$plugins = array();
