...
function loadforum() {
	...
	if($_G['setting']['forumpicstyle']) {
		...
		empty($_G['setting']['forumpicstyle']['thumbwidth']) && $_G['setting']['forumpicstyle']['thumbwidth'] = 218;
		empty($_G['setting']['forumpicstyle']['thumbheight']) && $_G['setting']['forumpicstyle']['thumbheight'] = 163;
	} else {
		$_G['setting']['forumpicstyle'] = array('thumbwidth' => 218, 'thumbheight' => 163);
	}
	if($fid) {
...
function convertunusedattach($aid, $tid, $pid) {
	...
	$attach['tid'] = $tid;
	$attach['pid'] = $pid;
	DB::insert(getattachtablebytid($tid), $attach);
