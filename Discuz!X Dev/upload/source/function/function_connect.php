...
function connect_get_request_token() {
	...
	$extra = array (
		'oauth_callback' => rawurlencode($_G['connect']['callback_url'] . '&referer=' . urlencode($_G['gp_referer']))
	);
	$sig_params = connect_get_oauth_signature_params($extra);
...
function connect_get_oauth_signature($url, $params, $method = 'POST', $oauth_token_secret = '') {
	...
	if(function_exists('hash_hmac')) {
		...
		$signature = connect_custom_hmac('sha1', $base_string, $key, true);
	}
	return $signature;
}
...
function connect_parse_bbcode($bbcode, $fId, $pId, $isHtml, & $attachImages = array ()) {
...
function connect_parse_attach($content, $fId, $pId, & $attachImages = array ()) {
	global $_G;

	$permissions = connect_get_user_group_permissions(array (
		7
	));
