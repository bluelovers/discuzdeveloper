...
class block_thread {
	...
	function getdata($style, $parameter) {
		...
		if($style['getpic'] || $picrequired) {
			$sqlfrom .= " INNER JOIN `".DB::table('forum_threadimage')."` ti ON t.tid=ti.tid AND ti.tid>0";
		...
		if($tagkeyword) {
			$sqlfrom .= " INNER JOIN `".DB::table('common_tagitem')."` ti ON ti.itemid=t.tid AND ti.idtype='tid'";
		}
