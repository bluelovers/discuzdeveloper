	...
	function on_register() {
		...
		if(!submitcheck('regsubmit', 0, $seccodecheck, $secqaacheck)) {
			...
			if(!$activation) {
				...
				if(!empty($this->setting['checkuinlimit']) && !empty($_G['gp_uin'])) {
					if($uinlimit) {
						showmessage('connect_register_uinlimit', '', array('limit' => $this->setting['connect']['register_uinlimit']));
					}
					if(!$this->setting['regconnect']) {
						showmessage('connect_register_closed');
					}
					if($user = DB::fetch_first("SELECT username FROM ".DB::table('common_member')." WHERE conuin='$_G[gp_uin]' AND conisbind='1'")) {
						showmessage('connect_register_bind_uin_already', '', array('username' => $user['username']));
					}
					$password = md5(md5(time().rand()));
				}
			}
