...
if($op == 'updateblock') {
	...
	if($start<0) $start = 0;
	$theurl = 'portal.php?mod=portalcp&ac=portalblock&op='.$op;
	...
	if($op == 'recommend') {
		...
		$theurl .= '&idtype='.$_GET['idtype'];
		$rewhere = array();
		switch ($_GET['idtype']) {
			case 'tid' :
				$rewhere[] = "(blockclass='forum_thread' OR blockclass='forum_attachment')";
				break;
			case 'gtid' :
				$rewhere[] = "(blockclass='group_thread' OR blockclass='group_attachment')";
				break;
			case 'blogid' :
